...
export default function MockInterview() {
  ...
  async function askQuestion(idx: number, qs: QuestionItem[]) {
    ...
    setDisplayedQ(""); setLive(""); setFinal(""); setError(null);
    answerRef.current = "";
    try {
      setPhase("speaking");
  ...
  function handleDoneAnswering() {
    ...
    const updated: QAPair[] = [...history, { question: currentQ!.question, topic: currentQ!.topic, answer }];
    setHistory(updated);
    setPhase("processing");
  ...
  const bannerMap: Record<Phase, { text: string; cls: string; spin?: boolean }> = {
    generating: { text: "Loading questions…",                        cls: "border-[#202026] bg-[#090b10] text-neutral-400", spin: true },
    speaking:   { text: "AI is speaking…",                          cls: "border-[#7aecc4]/20 bg-[#7aecc4]/5 text-[#7aecc4]" },
    listening:  { text: "Recording — click Done when finished",     cls: "border-red-600/20 bg-red-600/5 text-red-400" },
    processing: { text: "Processing…",                              cls: "border-[#202026] bg-[#090b10] text-neutral-400", spin: true },
    done:       { text: "Interview complete! Generating feedback…", cls: "border-[#7aecc4]/20 bg-[#7aecc4]/5 text-[#7aecc4]" },
  };
      ...
      <div className="min-h-screen bg-black text-white flex flex-col">
        ...
        <header className="flex items-center justify-between gap-4 px-6 py-4 border-b-2 border-[#202026] bg-black">
          ...
          <div className="flex items-center gap-3 flex-1 max-w-xs mx-auto">
            <div className="flex-1 h-2 rounded-full bg-[#202026] overflow-hidden">
              <div
          ...
          <div className="flex items-center gap-1.5 text-xs font-semibold text-neutral-500 uppercase tracking-wide">
            <span className="animate-pulse-dot w-2 h-2 rounded-full bg-red-500 inline-block" />
            Live
          </div>
        ...
        <main className="flex-1 flex flex-col gap-5 px-6 py-6 max-w-5xl w-full mx-auto">
          ...
          <div className="grid grid-cols-2 gap-4 max-sm:grid-cols-1">
            ...
            <div className={`v-card relative rounded-2xl overflow-hidden border-2 border-[#202026] bg-[#090b10] aspect-[16/10] flex items-center justify-center transition-all duration-300 ${aiSpeaking ? "speaking glow-ai border-[#7aecc4]/30" : ""}`}>
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-xs font-semibold text-neutral-500 uppercase tracking-wide">AI Interviewer</p>
              </div>
              ...
              <div className={`absolute top-3 right-3 px-2.5 py-1 rounded-full text-xs font-semibold border-2 transition-all duration-300 ${aiSpeaking ? "border-[#7aecc4]/25 bg-[#7aecc4]/10 text-[#7aecc4]" : "border-[#202026] bg-black/80 text-neutral-500"}`}>
            ...
            <div className={`v-card user-card user-card-bg relative rounded-2xl overflow-hidden border-2 border-[#202026] aspect-[16/10] flex items-center justify-center transition-all duration-300 ${userSpeaking ? "speaking glow-user border-red-600/30" : ""}`}>
              ...
              <div className="relative z-[1] flex flex-col items-center gap-3">
                ...
                <p className="text-xs font-semibold text-neutral-500 uppercase tracking-wide">You</p>
              </div>
              ...
              <div className={`absolute top-3 right-3 px-2.5 py-1 rounded-full text-xs font-semibold border-2 transition-all duration-300 ${userSpeaking ? "border-red-600/25 bg-red-600/10 text-red-400" : "border-[#202026] bg-black/80 text-neutral-500"}`}>
          ...
          <div className="rounded-2xl border-2 border-[#202026] bg-[#090b10] px-5 py-4 min-h-[110px] flex flex-col gap-3">
            <div className="flex items-center justify-between">
              ...
              {currentQ && (
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${phase === "listening" ? "bg-red-600/10 text-red-400" : "bg-[#7aecc4]/10 text-[#7aecc4]"}`}>
            ...
            {(phase === "speaking" || phase === "listening" || phase === "processing" || phase === "done") && displayedQ ? (
              <p className="text-base font-semibold text-white leading-relaxed">
                {displayedQ}
                {typingQ && <span className="animate-blink inline-block w-[2px] h-[1em] bg-[#7aecc4] ml-1 align-middle rounded-sm" />}
              </p>
            ) : phase === "generating" ? (
              <p className="text-sm text-neutral-600 italic">Loading your questions…</p>
            ) : (
            ...
            {phase === "listening" && (finalAnswer || liveTranscript) && (
              <p className="text-sm text-neutral-400 italic">
                {finalAnswer && <span className="text-white not-italic font-medium">{finalAnswer} </span>}
                {liveTranscript}
              </p>
            )}
          </div>
          ...
          <div className="flex items-center justify-center gap-3 pb-2">
            <button
              ...
              className={`w-12 h-12 rounded-full flex items-center justify-center text-lg border-2 transition-all ${phase === "listening" ? "animate-mic-pulse border-red-600/30 bg-red-600/10" : "border-[#202026] bg-[#090b10] hover:border-[#2a2a32]"}`}
            >
              {muted ? <MicOff size={20} color="#421717ff" /> : <Mic size={20} color="#7aecc4" />}
            </button>
            ...
            >
              {qIndex + 1 >= TOTAL_QUESTIONS ? "Finish interview →" : "Next question"}
            </button>
