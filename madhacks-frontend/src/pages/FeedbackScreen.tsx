        ...
        breakdown: [
          {
            ...
            topic: "Arrays / HashMaps", score: 85,
          ...
          },
          {
            ...
            topic: "Sliding Window", score: 58,
            remark: "Vague description. Did not differentiate fixed vs variable window, or explain the key invariant idea.",
            suggestedAnswer: "Sliding window maintains a subarray [l, r] and expands/shrinks it based on a constraint. Fixed window: move both pointers together. Variable window: expand r, shrink l when constraint violated.",
          },
          {
            ...
            topic: "Dynamic Programming", score: 70,
            remark: "Correct at a high level. Missing call stack implications of memoization and when to prefer each.",
            suggestedAnswer: "Memoization = top-down DP: solve recursively, cache results in a map. Tabulation = bottom-up DP: build a table iteratively. Memoization risks stack overflow for deep recursion.",
          },
          {
            ...
            topic: "Algorithms", score: 80,
            remark: "Good answer. Should have mentioned O(n²) worst case for quicksort and stability as a criterion.",
            suggestedAnswer: "Quicksort: O(n log n) avg, O(n²) worst — in-place. Merge sort: O(n log n), stable — use for linked lists. Heapsort: O(n log n), in-place, not stable.",
          },
        ],
      };
      setFeedback(demo);
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-black text-white">
        <div className="mx-auto w-full max-w-4xl px-4 py-10 sm:px-6 sm:py-14 flex flex-col gap-6">

          {/* ── Top bar ── */}
          <div className="flex items-center justify-between gap-4">
            <p className="text-sm font-semibold tracking-wide text-[#7aecc4]">MOCK INTERVIEW · FEEDBACK</p>
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="inline-flex items-center justify-center bg-[#7aecc4] text-black tracking-wide rounded-xl px-5 py-2 text-sm font-semibold transition hover:bg-[#1c2b2b] hover:text-white active:scale-[0.99]"
              >
                Dashboard →
              </button>
            </div>
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-5">
              ...
              <p className="text-xs font-semibold text-neutral-600 uppercase tracking-wide">Analysing your answers…</p>
            </div>
          )}

          {/* ── Error ── */}
          {!loading && error && (
            <div className="text-center text-red-400 text-sm font-semibold">⚠ {error}</div>
          )}
          ...
          {!loading && feedback && (
            <>
              ...
              <div className="animate-fade-up rounded-2xl border-2 border-[#202026] bg-[#090b10] p-6 sm:p-8">
                <p className="text-xs font-semibold text-neutral-500 uppercase tracking-wide mb-5">Overall Score</p>
                <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                  ...
                  <div className="relative w-[110px] h-[110px] flex-shrink-0 flex items-center justify-center flex-col">
                    <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full" style={{ transform: "rotate(-90deg)" }}>
                      ...
                      <circle
                        className="score-ring-fill" cx="50" cy="50" r="45"
                        stroke={scoreColor(feedback.overallScore)}
                        strokeDashoffset={ringOffset(feedback.overallScore)}
                      />
                    </svg>
                    <span className="relative z-[1] text-[28px] font-bold leading-none" style={{ color: scoreColor(feedback.overallScore) }}>
                      {feedback.overallScore}
                    </span>
                    <span className="relative z-[1] text-[10px] font-semibold text-neutral-600 tracking-wide">/100</span>
                  </div>

                  {/* Text */}
                  <div className="flex flex-col gap-2 text-center sm:text-left">
                    <p className="text-xl font-bold" style={{ color: scoreColor(feedback.overallScore) }}>
              ...
              <div className="flex flex-col gap-3">
                <p className="text-xs font-semibold text-neutral-500 uppercase tracking-wide">Question Breakdown</p>
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <span className="hidden sm:block text-[10px] font-semibold px-2.5 py-1 rounded-full border-2 border-[#202026] bg-black text-neutral-500">
                            {item.topic}
                          </span>
                          <span
                            className="text-xs font-bold px-3 py-1 rounded-full border-2"
                            style={{ color: sc, borderColor: `${sc}44`, background: `${sc}12` }}
                          >
                            {item.score}
                          </span>
                          <span className={`bd-chevron text-neutral-600 text-sm ${isOpen ? "open" : ""}`}>▾</span>
                        </div>
                      </div>
                        ...
                        <div className="animate-fade-up-fast border-t-2 border-[#202026] px-5 pt-5 pb-6 flex flex-col gap-4">
                          ...
                          <div className="flex items-center gap-3">
                            <div className="flex-1 h-1.5 rounded-full bg-[#202026] overflow-hidden">
                              <div className="score-bar-fill h-full rounded-full" style={{ width: `${item.score}%`, background: sc }} />
                            </div>
                            <span className="text-xs font-bold text-neutral-500">{item.score}/100</span>
                          </div>

                          {/* Your answer */}
                          <div className="flex flex-col gap-2">
                            <p className="text-[10px] font-bold uppercase tracking-widest text-neutral-600">Your Answer</p>
                            <div className="rounded-xl border-2 border-[#202026] bg-black px-4 py-3 text-sm text-neutral-400 leading-relaxed">
                              {item.answer}
                            </div>
                          </div>
                          ...
                          {/* Feedback */}
                          <div className="flex flex-col gap-2">
                            <p className="text-[10px] font-bold uppercase tracking-widest text-neutral-600">Feedback</p>
                          ...
                          {/* Suggested answer */}
                          <div className="flex flex-col gap-2">
                            <p className="text-[10px] font-bold uppercase tracking-widest text-neutral-600">Suggested Answer</p>
